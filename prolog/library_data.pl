...
most_borrowed_book(B) :-
    ...
    count_borrowed(B,Count),
     ...
     % maximize the count to get the most borrowed book
    not((book(Other,_),
     Other \= B, count_borrowed(Other, OtherCount),
      OtherCount > Count)),!.  % cut to get the most borrowed book (one only)
...
count_borrowed(Book,Seen,Acc,Count) :-
    borrowed(Student,Book),
    \+ member_book(borrowed(Student,Book),Seen),  % to remove the duplicates in the borrowed books list
    NewAcc is Acc + 1,
    count_borrowed(Book,[borrowed(Student, Book)|Seen],NewAcc,Count). 

% returns numbers of times the the book was borrowed
...
% uses accumilator to print the whole list at once
ratings_of_book(Book,Acc,Result):-
    rating(Student,Book,Score),
    \+ member_book((Student,Score),Acc),  % avoid duplicates
    ratings_of_book(Book,[(Student,Score)|Acc],Result),!.

ratings_of_book(_,Result,Result). % puts the the resulted list in Result
